...
// =============================================================================
// PDOK LOCATIESERVER CONFIGURATIE
// =============================================================================
...
const PDOKLocationSearch = ({ onLocationSelect, onClear, selectedLocation, colors, font }) => {
  ...
  // Debounced search - skip als query overeenkomt met geselecteerde locatie
  useEffect(() => {
    // Skip search als query exact overeenkomt met geselecteerde locatie (net geselecteerd)
    if (selectedLocation?.weergavenaam && query === selectedLocation.weergavenaam) {
      setSuggestions([]);
...
const IkStaSterkTest = () => {
  ...
  // =============================================================================
  // PDOK LOCATIESERVER FUNCTIES (NIEUW 2025)
  // =============================================================================
  ...
  const handleLocationKeyDown = (e) => {
    ...
    switch (e.key) {
      ...
      case 'Enter':
        e.preventDefault();
        if (selectedLocationIndex >= 0) handleLocationSelect(locationSuggestions[selectedLocationIndex]);
        break;
      case 'Escape':
        setShowLocationDropdown(false);
        break;
  ...
  const saveToDatabase = async (woonplaatsValue, emailValue, calculatedRiskLevel) => {
    try {
      const data = {
        ...
        // PDOK Locatie gegevens (NIEUW 2025)
        ...(locationData && {
          pdok_id: locationData.pdok_id,
          gemeentenaam: locationData.gemeentenaam,
          gemeentecode: locationData.gemeentecode,
          postcode: locationData.postcode,
          straatnaam: locationData.straatnaam,
          huisnummer: locationData.huisnummer,
          latitude: locationData.latitude,
          longitude: locationData.longitude,
          wijkcode: locationData.wijkcode,
          buurtcode: locationData.buurtcode,
        }),
  ...
  // GEUPDATE 2025: Gebruikt nu PDOK coordinaten indien beschikbaar
  const getSortedFysios = () => {
    // Gebruik PDOK coordinaten indien beschikbaar, anders fallback naar oude methode
    let coords = null;
  ...
  const renderDemographics = () => {
    ...
    <Card>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        {locationData && (
          <div style={{
            marginTop: '10px', padding: '10px 14px', background: ZLIM.successLight,
            borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px'
          }}>
            <MapPin size={16} color={ZLIM.success} />
            <span style={{ fontSize: '14px', color: ZLIM.successDark, fontWeight: FONT.medium, flex: 1 }}>
              {locationData.woonplaats || locationData.weergavenaam}
              {locationData.gemeentenaam && ` (${locationData.gemeentenaam})`}
            </span>
